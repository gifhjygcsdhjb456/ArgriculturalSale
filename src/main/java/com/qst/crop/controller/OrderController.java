...
//        System.out.println(orders);
    ...
    //根据登录用户查询我买的订单详情
    @ApiOperation(value = "查询销售账单")
    @GetMapping("/selectBuysDetail")
    public Result<List<PurchaseDetail2>> selectBuysDetail() {
        List<PurchaseDetail2> purchaseDetail = purchaseDetailService.selectByPurchaseId();
        return new Result<List<PurchaseDetail2>>(true, StatusCode.OK, "查询成功", purchaseDetail);
    }
    ...
    public Result addByKnowledge(@PathVariable("id") Integer id,@PathVariable("content") String content) {
        ...
        System.out.println("添加商品评论："+comment);
